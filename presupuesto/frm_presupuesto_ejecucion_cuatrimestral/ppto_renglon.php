...
<table width="100%"  border="0">
  ...
  <tr>
    ...
    <td height="25" class="tituloproducto"><select name="txt_codigo_mes">
        <?
		$consulta_mes = "select codigo_mes,nombre_mes from cat_mes where cuatrimestre = ".$c;
		$query_mes = mssql_query($consulta_mes);
		if ($query_mes)
		{
			while($rows = mssql_fetch_row($query_mes))			
			{
  ...
  <tr>
    <td height="25" class="titulomenu">Renglones</td>
    <td height="25" class="tituloproducto">
	<?
	...
	$vgrupo = $vector_grupo[0];
		...
		$query_renglones = "select 
									codigo_renglon 
							from 
									tb_asignacion_cuatrimestral 
							where 
									codigo_grupo = $vgrupo
									and codigo_periodo = $c
									and codigo_financiamiento_actividad = $pr
";

		$consulta_renglones = mssql_query($query_renglones);
		
		if ($consulta_renglones)
		{
			while($vector_renglones = mssql_fetch_row($consulta_renglones))
			{
				print $vector_renglones[0];
				print '.';
			}
