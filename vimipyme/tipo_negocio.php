...
/*	if ( $sstipo != 1)
 ...
 if ( isset($_POST['inserta']) && ($_POST['inserta'] == 1) ) 
	{
		$verifica = "select tipo_negocio from tipo_negocio where tipo_negocio = '$_POST[tiponeg]'";
		$resver = mssql_query($verifica);
		if (mssql_num_rows($resver) > 0) 
			{
			 envia_msg('El tipo de negocio que desea ingresar ya existe en el sistema');
			}
		else
			{
				$sql="insert into tipo_negocio (tipo_negocio) 
						values ('$_POST[tiponeg]') ";
...
<form name="form1" method="post" action="tipo_negocio.php" onSubmit="return Verifica()">
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr bgcolor="#0066CC">
    <td colspan="7"><div align="center"><span class="Estilo1 Estilo2"> CATALOGO <BR>TIPO DE NEGOCIO</span></div></td>
    </tr>
  <!--tr>
  ...
  <tr><Td><br></Td></tr>
  <tr class="Estilo1" >
    <td class="Estilo22" align="right">Tipo de Negocio<font color="#FF0000"><strong>**</strong></font></td>
    <td class="Estilo7"><input name="tiponeg" type="text" class="Estilo7" maxsize="50"  size="50" onKeyUp="javascript:this.value=this.value.toUpperCase();"></td>
  </tr>
