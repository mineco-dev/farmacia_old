...
<div align="left">
  <form name="form1" method="post" action="">
    ...
    (ESCRIBA NOMBRE O APELLIDO)</strong></strong></p>
            </div>
        </div></td>
      </tr>
    </table>
    <table class="tborder" cellpadding="0" cellspacing="1" border="0" width="100%" id="table17">
      <tr align="center" bgcolor="#006699" class="thead">
        ...
        <td width="51%" class="titulotabla"><strong>Nombres</strong></td>
		...
		<?
				...
				$consulta = "select USERID, NAME as empleado from USERINFO
								where NAME like '%$busqueda%'";//primer consulta
				...
				if (isset($_REQUEST["in"]))	
				{
					...
					if ($inicial!="all")
						$consulta = "select USERID, NAME as empleado from USERINFO
									where NAME like '$inicial%'"; //segunda consulta
							       
						else
							$consulta = "select USERID, NAME as empleado from USERINFO";//tercer consulta
				}
				

				else
				{
					$consulta = "select top 10 USERID, NAME as empleado from USERINFO
									where NAME like 'a%'"; //cuarta consulta
				}
				...
				while($row=$fetch_array($result))
				{
					...
					window.opener.document.getElementById('$tipo"."[".$posi."][1]').value = '".$row["USERID"]."';
					window.close();
					window.opener.focus(); 
