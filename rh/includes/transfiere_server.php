...
function get_tb_correo()
{
	$campos[0]  = "detalle";
	return $campos;
}

function get_tb_telefono()
{
	$campos[0]  = "numero_telefono";
	return $campos;
}
...
function transfiere_persona($contrato_detalle)
{
	...
	while($Fields=$dbmsp->MoveNext())
	{
		...
		$dbmsp22->QueryI();
		$max_persona = get_valor($dbmsp22,"max(codigo_persona_individual)","tb_persona","");
		
		$dbmsp22->sql= get_query($dbmsp2,"tb_correo_electronico",get_tb_correo(),"codigo_persona_individual","$max_persona","where
	                                codigo_persona_individual=$codigo_persona");
		$dbmsp22->QueryI();
		$dbmsp22->sql= get_query($dbmsp2,"tb_telefono",get_tb_telefono(),"codigo_persona_individual","$max_persona","where
	                                codigo_persona_individual=$codigo_persona");
		$dbmsp22->QueryI();
