...
<table width="100%"  border="0">
  <tr>
    <td width="11%" height="25"><div align="center"><img src="../../images/" width="150" height="95"></div></td>
    ...
    <td width="12%"><div align="right"><img src="../../images/hardware_software.jpg" width="112" height="113"></div></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	{	
		if (isset($txt_obj)) //verifico si hay objeto seleccionado
		{
			...
			conectardb($inventarioadmin);
				...
				{
						$txt_cat=$_REQUEST["txt_cat"]; //categoria de los vehiculos
						$txt_subcat=$_REQUEST["txt_subcat"]; //para vehiculos para motos es la 4
						$objeto=$_REQUEST["txt_obj"]; // para automoviles						
						$qry_insert="insert into $tabla_destino(codigo_categoria, codigo_subcategoria, codigo_objeto, ";
						$cnt=1;
						while ($cnt<=count($campo)) //concatena lista de campos a grabar
						{
						...
						$qry_insert.="usuario_creo, fecha_creado, activo) values ('$txt_cat', '$txt_subcat', '$objeto', ";			
						$cnt=1;			
						while ($cnt<=count($campo))  //concatena el contenido a grabar en los campos
						{
							...
							if ($tipo_campo[$cnt]=='3')
							{
								if (isset($_REQUEST["$campo[$cnt]"])) $variable='SI';
								else $variable='NO';
							}
							else
							{
						...
						if ($res_insert)
						{
							echo '<tr><td class="titulocategoria" colspan="3" align="center">EL REGISTRO SE HA GRABADO CORRECTAMENTE<br><br>Para ingresar otro registro <a href="agregar.php">[HAGA CLIC AQUI]</a></td></tr>';															
	...
	{
		echo '<tr><td class="error" colspan="3" align="center">Para continuar ingresando vehiculos <a href="agregar.php">[HAGA CLIC AQUI]</a></td></tr>';
	}
	?>            
	</td>
  </tr>
