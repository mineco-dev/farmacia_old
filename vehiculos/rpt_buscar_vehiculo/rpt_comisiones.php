...
<table width="90%"  border="0" align="center">
  <tr>
    ...
    <td width="72%"><p align="center" class="titulocategoria Estilo1">SUBGERENCIA ADMINISTRATIVA</p>
	  ...
	  <p align="center" class="titulocategoria"> HISTORIAL DE COMISIONES REALIZADAS POR VEHICULO</p></td>
		...
		{			
			...
			conectardb($vehiculos);
				...
				{			
					...
					if ((($row_qry_plantilla["codigo_tipo_campo"]==8) || ($row_qry_plantilla["codigo_tipo_campo"]==10)) && ($status==''))
					{							
						$etiqueta[$no_campos_detalle]=$row_qry_plantilla["etiqueta"];	//etiqueta para la tabla que detalla los registros ingresados.
						$campo_detalle=$row_qry_plantilla["tb_destino"].'.'.$row_qry_plantilla["campo"]; //campo para concatenar al qry.				
				...
				$qry_detalle="SELECT tb_salida_vehiculo.codigo_salida_vehiculo, tb_salida_vehiculo.fecha_inicial, tb_salida_vehiculo.fecha_final, tb_departamento.nombre_departamento, 
				tb_municipio.nombre_municipio, tb_zona.numero_zona, tb_salida_vehiculo.direccion, 
				tb_motivo_salida_vehiculo.motivo_salida_vehiculo, tb_salida_vehiculo.fecha_salida, tb_salida_vehiculo.fecha_entrada, tb_salida_vehiculo.kilometraje_salida, 
				tb_salida_vehiculo.kilometraje_entrada, cat_tanque.tanque, r.tanque as tanque_devuelto, 
				tb_salida_vehiculo.usuario_opero_entrada, tb_salida_vehiculo.activo 
				FROM tb_salida_vehiculo 
				LEFT JOIN tb_departamento ON tb_salida_vehiculo.codigo_departamento=tb_departamento.codigo_departamento 
				LEFT JOIN tb_municipio ON tb_salida_vehiculo.codigo_municipio=tb_municipio.codigo_municipio 
				LEFT JOIN tb_zona ON tb_salida_vehiculo.codigo_zona= tb_zona.codigo_zona 
				LEFT JOIN tb_motivo_salida_vehiculo ON tb_salida_vehiculo.codigo_motivo_salida_vehiculo=tb_motivo_salida_vehiculo.codigo_motivo_salida_vehiculo 
				LEFT JOIN cat_tanque ON tb_salida_vehiculo.codigo_tanque=cat_tanque.codigo_tanque 
				LEFT JOIN cat_tanque r ON tb_salida_vehiculo.codigo_tanque_recibido=r.codigo_tanque 
				WHERE tb_salida_vehiculo.codigo_inventario_enc='$id' 
				ORDER BY tb_salida_vehiculo.fecha_entrada, tb_salida_vehiculo.fecha_salida, tb_departamento.nombre_departamento, tb_municipio.nombre_municipio, tb_zona.numero_zona, 
				tb_salida_vehiculo.direccion, tb_motivo_salida_vehiculo.motivo_salida_vehiculo, tb_salida_vehiculo.kilometraje_salida, 
				tb_salida_vehiculo.kilometraje_entrada, cat_tanque.tanque, tb_salida_vehiculo.usuario_opero_entrada";
				...
				{		 			 					 					 
					...
					if (!$hay_detalle)
					{
						echo '<tr><td class="error" colspan="'.$no_campos_detalle.'" align="center">NO SE ENCONTRARON REGISTROS DE COMISIONES<br>CON ESTE VEH�CULO</td></tr>';
					}	
			}
