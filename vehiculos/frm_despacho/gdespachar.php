...
<table width="100%"  border="0">
  ...
  <tr>
	...
	{			
		...
		if (($acceso>=4) && ($acceso<=8))
		{
			...
			while ($cnt<=count($campo)) //concatena lista de campos a grabar
				...
				$qry_insert.="usuario_opero_salida='$nombre_usuario', fecha_opero_salida=getdate(), fecha_salida=getdate(), activo=1, codigo_estado_salida_vehiculo=4 where $campo_llave_destino=$txt_id";														
				//session_unregister("ingresando_obj");	
				conectardb($vehiculos);										
				$res_insert=$query($qry_insert);			
				if ($res_insert)
				{					
					echo '<tr><td class="titulocategoria" colspan="3" align="center">SE HA OPERADO LA SALIDA DEL VEHICULO CORRECTAMENTE<br><br>Para operar otro registro <a href="buscar/buscar.php">[HAGA CLIC AQUI]</a></td></tr>';															
	...
	{
		echo '<tr><td class="error" colspan="3" align="center">Para despachando solicitudes <a href="buscar/buscar.php">[HAGA CLIC AQUI]</a></td></tr>';
	}
	?>            
	</td>
  </tr>
