	...
	{	
		if (isset($txt_obj)) //verifico si hay objeto seleccionado
		{
			...
			if ($existe==false)
			{
				...
				$qry_insert="insert into tb_inventario(codigo_objeto, ";
				$cnt=1;
				...
				$qry_insert.="usuario_creo, fecha_creado) values ('$txt_obj', ";			
				$cnt=1;			
				...
				$qry_insert.="'$nombre_usuario', getdate())";
				...
				if ($res_insert)
				{
					...
					if ($txt_obj==2)  
					{
					  ...
					  {						
							...
							$qry_lector.="(codigo_inventario_enc, codigo_tipo_lector, serie, codigo_velocidad_lector, codigo_marca, modelo) ";
							$qry_lector.="values ($codigo_inventario_enc, '$tipolector[$cnt]', '$serielector[$cnt]', '$velocidadlector[$cnt]', '$marcalector[$cnt]', '$modelolector[$cnt]')";			
							$query($qry_lector);
							$cnt++;
					  }
					  ...
					  {						
							...
							$qry_install.="(codigo_inventario_enc, codigo_tipo_software, codigo_casa_software, codigo_version, codigo_idioma, serie,  licencia) ";
							$qry_install.="values ($codigo_inventario_enc, '$tipoinstall[$cnt]', '$fabricante[$cnt]', '$versioninstall[$cnt]', '$idiomainstall[$cnt]', '$serieinstall[$cnt]', '$conlicencia')";			
							$query($qry_install);
							$cnt++;
					  }					  					 			  
