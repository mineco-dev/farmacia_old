...
e.fecha_creado,  e.observaciones, dep.nombre,  e.nombre_solicitante, d.cantidad_solicitada,
d.codigo_egreso_enc, b.bodega,
 d.cantidad_entregada from tb_egreso_det d
inner join tb_egreso_enc e on
e.codigo_egreso_enc = d.codigo_egreso_enc
...
where d.codigo_egreso_enc = '$idsolicitud' and d.activo=1";
	
	$res_qry_producto=$query($Fields);
		while($row=$fetch_array($res_qry_producto))
		{				
			
			$codigo=$row["codigo_egreso_enc"];
...
<div id="TabbedPanels1" class="TabbedPanels">
  <ul class="TabbedPanelsTabGroup">
    <li class="TabbedPanelsTab" tabindex="0"><strong>Detalles de Egresos</strong></li>
    </ul>
   ...
   // Mostrar resultados de la consulta
	 ...
	 $nfilas = mssql_num_rows ($consulta);
      if ($nfilas > 0)
      {
		 ...
		 print ("<TH>Cantidad Entregada</TH>\n");
         ...
         for ($i=0; $i<$nfilas; $i++)
         {
			...
			print ("<TD>" . $resultado['cantidad_entregada'] . "</TD>\n");
