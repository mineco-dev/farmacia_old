...
if(validaSelect($selectDestino) && validaOpcion($opcionSeleccionada))
{
	...
	{												
		...
		$consulta=mssql_query("SELECT distinct p.codigo_producto, (p.producto +' - '+ p.marca +' EN '+ m.unidad_medida) as producto,p.activo, p.codigo_subcategoria, p.codigo_categoria 
					FROM     cat_producto p
					...
					and p.codigo_subcategoria=i.codigo_subcategoria and p.codigo_categoria=i.codigo_categoria --and i.codigo_bodega= 15
					where i.codigo_subcategoria = '$opcionSeleccionada' and p.activo = 1 and i.codigo_empresa = ".$_SESSION["empresax"]."
					and i.codigo_categoria = $cbo_categoria order by p.codigo_producto");	
							
							// where i.codigo_subcategoria = '$opcionSeleccionada' and p.activo = 1 and i.codigo_empresa =  and i.codigo_bodega =12
		...
		echo "<option value='".$registro[0]."'>".$registro[0]."  ---  ".utf8_encode($registro[1])."</option>";
		}			
		echo "</select>";
	}	
	...
	{	
		...
		while($registro=mssql_fetch_row($consulta))
		{
			...
			// Imprimo las opciones del select
			echo "<option value='".$registro[1]."'>".$registro[1]."  ---  ".utf8_encode($registro[2])."</option>";
		}			
		echo "</select>";
	}	
}
