    ...
    <table class="tborder" cellpadding="0" cellspacing="1" border="0" width="100%" id="table17">
      <tr align="center" bgcolor="#006699" class="thead">
        ...
        <td width="20%" class="titulotabla"><strong>Bodega</strong></td>
            <td width="20%" class="titulotabla"><strong>Empresa</strong></td>
      </tr>
		<?PHP
...
LEFT OUTER JOIN cat_empresa e ON e.codigo_empresa = i.codigo_empresa
							where (p.producto like '%$busqueda%' or p.uso like '%$busqueda%' or e.empresa like '%$busqueda%') and p.activo=1";																		
				}
				else	
...
LEFT OUTER JOIN cat_empresa e ON e.codigo_empresa = i.codigo_empresa
				...
				while($row=$fetch_array($result))
				{
					...
					if (($existencias==0) && ($egresando==true))
					{
						echo '<tr class='.$clase.'>';
						echo '<td>&nbsp;</td>';										
						echo '<td>'.$row["producto"].'</td><td>'.$row["existencia"].'</td><td>'.$row["bodega"].'</td><td>'.$row["empresa"].'</td></tr>';																				
					}
					else
					{
						echo '<tr class='.$clase.'>';
