...
class tc_calendar{
	...
	function writeScript(){
		...
		if($this->date_picker){
			...
			if($this->icon){
				echo("<img src=\"".$this->icon."\" border=\"0\" align=\"absmiddle\" width=\"30px\" height=\"30px\" />");
			}else echo($this->txt);				
			echo("</a>");
			$div_display = "none";
		...
		echo("<div id=\"div_".$this->objname."\" style=\"position:relative;display:".$div_display.";z-index:10000;\"><IFRAME id=\"".$this->objname."_frame\" style=\"DISPLAY:block; LEFT:0px; POSITION:".$iframe_position."; TOP:0px;\" src=\"".$this->path."../includes/calendar_form.php?objname=$this->objname&selected_day=".$this->day."&selected_month=".$this->month."&selected_year=".$this->year."\" frameBorder=\"0\" scrolling=\"no\" height=\"300\" width=\"300\"></IFRAME></div>");
	}
	
	//write the select box of days
	function writeDay(){
		echo("<select class=\"form-control\" style=\"width:12%;\" name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" onChange=\"javascript:tc_setDay('".$this->objname."', this[this.selectedIndex].value);\">");
	...
	function writeMonth(){
		echo("<select class=\"form-control\" style=\"width:12%;\"name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" onChange=\"javascript:tc_setMonth('".$this->objname."', this[this.selectedIndex].value);\">");
	...
	function writeYear(){
		echo("<input class=\"form-control\" style=\"width:12%;\" type=\"textbox\" name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" value=\"$this->year\" maxlength=4 size=5 onBlur=\"javascript:tc_setYear('".$this->objname."', this.value);\"> ");
	}
	
	//write hidden components
	function writeHidden(){
		$svalue = $this->year."-".$this->month."-".$this->day;
		echo("<input  class=\"form-control\" style=\"width:12%;\" type=\"hidden\" name=\"".$this->objname."\" id=\"".$this->objname."\" value=\"$svalue\">");
		echo("<input  class=\"form-control\" style=\"width:12%;\" type=\"hidden\" name=\"".$this->objname."_dp\" id=\"".$this->objname."_dp\" value=\"".$this->date_picker."\">");
	}
}
?>