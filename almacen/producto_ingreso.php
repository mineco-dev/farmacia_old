  ...
  <form name="form1" method="post" action="">
    ...
    <table class="tborder" cellpadding="0" cellspacing="1" border="0" width="100%" id="table17">
      <tr align="center" bgcolor="#006699" class="thead">
        ...
        <td width="10%" class="titulotabla"><strong>Codigo Categoria</strong></td>
         <td width="10%" class="titulotabla"><strong>Codigo Subcategoria</strong></td>
      </tr>
		<?PHP
...
INNER JOIN cat_medida m ON p.codigo_medida = m.codigo_medida 
				...
				while($row=$fetch_array($result))
				{
					...
					$existencias=$row["codigo_producto"];
					$clase = "detalletabla2";
					if ($i % 2 == 0) 
					{
						$clase = "detalletabla1";
					...
					if (($existencias==0) && ($egresando==true))
					{
						echo '<tr class='.$clase.'>';
						echo '<td>&nbsp;</td>';										
						echo '<td>'.$row["producto"].'</td><td>'.$row["codigo_producto"].'</td><td>'.$row["codigo_categoria"].'</td><td>'.$row["codigo_subcategoria"].'</td></tr>';																				
					}
					else
					{
						echo '<tr class='.$clase.'>';
