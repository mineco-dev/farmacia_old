         ...
         <tr>
              <td valign="top">&nbsp;</td>
               ...
               <td colspan="2">
                  <?PHP
				  	conectardb($almacen);
					$qry_empresa="select * from Tb_Dependencias";	
					$res_qry_empresa=$query($qry_empresa);					
					?>
					<select name="cbo_dependencias" id="cbo_dependencias" onChange="javascript:cargarCombo('cbo_jefe.php', 'cbo_dependencias', 'Div_jefe')">
					<?PHP
					$nombre=":: Seleccione ::";
					echo'<option value="0">'.$nombre.'</option>';
					while($row_empresa=$fetch_array($res_qry_empresa))
					{
						echo'<option value="'.$row_empresa["Codigo_Depen"].'">'.$row_empresa["Descripcion_Depen"].'</option>';
					}
					?>
					</select>
					<?				
          ...
          <td colspan="2">&nbsp;</td>
            ...
            <tr>
                ...
                <td colspan="2">
				<div id="Div_jefe">
		  		<select name="cbo_jefe"  id="cbo_jefe">
		  		</select>
	 			 </div>
                  </td>
                  </tr>
...
{
//////////////////////// Encabezado ///////////////////////////////////////////////////
	if ((form['cbo_dependencias'].value) == "0"){alert('Seleccione el tipo de documento que respalda el ingreso'); form['cbo_dependencias'].focus();  return};	
    if ((form['nombre[0][2]'].value) == ""){alert('Seleccione el nombre del solicitante'); form['nombre[0][2]'].focus();  return};
