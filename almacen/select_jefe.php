...
<form name="frm_select_jefe" id="frm_select_jefe" action="aprobacion.php" method="post">
    ...
    <div class="TabbedPanelsContent">
      <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="panel">
        ...
        <tr>
          ...
          <td valign="top">Escoje Su Nombre</td>
          <td colspan="2">
			 	 <?PHP
		  	conectardb($almacen);											
					$qry_tipo_empresa="select j.Codigo_Jefe_Depen, (j.Nombre_Jefe_Depen +' - '+ d.Descripcion_Depen) as Nombre from tb_dependencias d 
inner join tb_jefes_depen j
on j.Codigo_Dependencia = d.Codigo_Depen order by Nombre";										
					$res_qry_tipo_empresa=$query($qry_tipo_empresa);	
					   
					echo('<select name="cbo_jefe">');
					$nombre=":: Seleccione ::";
					//echo'<option value="0">'.$nombre.'</option>';
					while($row_tipo_empresa=$fetch_array($res_qry_tipo_empresa))
					{
						echo'<option value="'.$row_tipo_empresa["Codigo_Jefe_Depen"].'">'.$row_tipo_empresa["Nombre"].'</option>';
...
{
//////////////////////// Encabezado ///////////////////////////////////////////////////
	if ((form['cbo_jefe'].value) == "0"){alert('Seleccione el tipo de documento que respalda el ingreso'); form['cbo_jefe'].focus();  return};	
