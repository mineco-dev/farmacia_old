...
// Valores iniciales para la paginacion
	...
	$rangofin = 10;
	$sqlsel = mssql_query("select a.id_subgrupo, a.subgrupo, b.grupo from grupo b, subgrupo a where a.id_grupo=b.id_grupo");
 ...
 if ( isset($_POST['inserta']) && ($_POST['inserta'] == 1) ) 
	{
		...
		if (mssql_num_rows($resver) > 0) 
			...
			{
				$sql="insert into subgrupo (subgrupo, id_grupo) 
						values ('".$_POST['subgrupo']."',".$_POST['grupo'].") ";
...
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr class="Estilo1" >
    ...
    <td class="Estilo7"><input name="subgrupo" type="text" class="Estilo7" maxsize="150"  size="100" onKeyUp="javascript:this.value=this.value.toUpperCase();"></td>
  </tr>
...
//	$sqlsel = "select id_grupo, grupo from grupo where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
	$sqlsel = "select a.grupo, a.ident_grupo, b.id_subgrupo, b.subgrupo from grupo a, subgrupo b where a.id_grupo = b.id_grupo ";//where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
//	echo $sqlsel;
//	$sqlsel = "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id_grupo) as row FROM grupo ) as grupo WHERE row > $rangoini and row <= $rangofin";
	...
	<tr>
		...
		<td bgcolor="#99CCFF"><? echo $row["id_subgrupo"].' '.$row["subgrupo"]; ?></td>
		<td bgcolor="#C9CDED"><a href="edicion_subgrupo.php?tabla=subgrupo&pag=subgrupo.php&id=<? echo $row["id_subgrupo"]; ?>" title="Editar" target="mainFrame"><img src="images/iconos/b_edit.png" width="16" height="16" border="0"></a></td>
	    <td bgcolor="#99CCFF"><a href="subgrupo.php?eli=1&id=<? echo $row["id_subgrupo"]; ?>" title="Eliminar" target="_self"><img src="images/iconos/button_drop.png" width="16" height="16" border="0"></a></td>
	</tr>
	<?
 		}
