...
// Valores iniciales para la paginacion
	...
	$sqlsel_g = "select a.id_actividad, a.actividad, b.programa from programa b, actividad a where a.id_programa=b.id_programa";	
	$resu_g = mssql_query($sqlsel_g);
	$Total = mssql_num_rows($resu_g);
	$maxpag = ceil($Total / 10);	

	$sqlsel = "select top 10 a.id_actividad, a.actividad, b.programa from programa b, actividad a where a.id_programa=b.id_programa and a.id_actividad BETWEEN $rangoini and $Total";
	$resu = mssql_query($sqlsel);
/*	$Total = mssql_num_rows($resu);
	$maxpag = ceil($Total / 10);	*/
...
<form name="form3" method="post" action="#">
  <table border="1" align="center"bgcolor="#C9CDED">
    <tr class="Estilo1">
	
		<?
			if ($PagNow != 1)
			{
				echo "<td><a href=\"actividad.php?linkant=actividad.php&pag=-1&maxpag=".$maxpag."\">Anterior</a></td>";
			}
			if ($maxpag > 10)
			{
