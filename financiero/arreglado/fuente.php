...
//	$sqlsel = "select id_grupo, grupo from grupo where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
	$sqlsel = "select id_fuente, fuente, denominacion from fuente_financiamiento ";//where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
//	echo $sqlsel;
//	$sqlsel = "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id_grupo) as row FROM grupo ) as grupo WHERE row > $rangoini and row <= $rangofin";
	...
	<tr>
	    ...
	    <td bgcolor="#99CCFF"><a href="grupo.php?eli=1&id=<? echo $row["id_fuente"]; ?>" title="Eliminar" target="mainFrame"><img src="images/iconos/button_drop.png" width="16" height="16" border="0"></a></td>
	</tr>
	<?
 		}
