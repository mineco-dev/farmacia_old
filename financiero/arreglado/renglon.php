...
// Valores iniciales para la paginacion
	...
	$rangofin = 10;
	$sqlsel = mssql_query("select a.id_renglon, a.renglon, b.subgrupo from subgrupo b, renglon a where a.id_subgrupo=b.id_subgrupo");
...
<table border="0" align="center" class="Estilo1 ">
  <tr>
	...
	<td bgcolor="#99CCFFF" width="500" align="center"><strong>Rengl�n</strong></td>
...
//	$sqlsel = "select id_grupo, grupo from grupo where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
	$sqlsel = "select a.grupo, a.ident_grupo, b.id_subgrupo, b.subgrupo, b.ident_subgrupo, c.ident_renglon, c.renglon from grupo a, subgrupo b, renglon c where b.id_subgrupo = c.id_subgrupo and a.id_grupo = b.id_grupo ";//where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
//	echo $sqlsel;
//	$sqlsel = "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id_grupo) as row FROM grupo ) as grupo WHERE row > $rangoini and row <= $rangofin";
	...
	<tr>
		...
		<td bgcolor="#99CCFF"><? echo $row["ident_renglon"].' '.$row["renglon"]; ?></td>
	    ...
	    <td bgcolor="#99CCFF"><a href="subgrupo.php?eli=1&id=<? echo $row["id_renglon"]; ?>" title="Eliminar" target="self"><img src="images/iconos/button_drop.png" width="16" height="16" border="0"></a></td>
	</tr>
	<?
 		}
