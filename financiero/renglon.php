...
// Valores iniciales para la paginacion
	...
	$sqlsel_g = "select a.id_renglon, a.renglon, b.subgrupo from subgrupo b, renglon a where a.id_subgrupo=b.id_subgrupo";	
	$resu_g = mssql_query($sqlsel_g);
	$Total = mssql_num_rows($resu_g);
	$maxpag = ceil($Total / 10);	
	
//	$sqlsel = "select top 10 a.id_renglon, a.renglon, b.subgrupo, c.grupo, c.ident_grupo from grupo c, subgrupo b, renglon a where a.id_subgrupo=b.id_subgrupo and a.id_renglon BETWEEN $rangoini and $Total";
	$sqlsel = "select top 10 c.id_renglon, a.grupo, a.ident_grupo, b.id_subgrupo, b.subgrupo, b.ident_subgrupo, c.ident_renglon, c.renglon 
				from grupo a, subgrupo b, renglon c where b.id_subgrupo = c.id_subgrupo and a.id_grupo = b.id_grupo and c.id_renglon BETWEEN $rangoini and $Total order by 1, 4, 8 asc" ;
...
//	 cambiar_ventana('mtlogin.php');
 ...
 if ( isset($_POST['inserta']) && ($_POST['inserta'] == 1) ) 
	{
		$verifica = "select renglon from renglon where renglon = '".$_POST['renglon']."'";
		$resver = mssql_query($verifica);
		if (mssql_num_rows($resver) > 0) 
			{
			 envia_msg('El renglon que desea ingresar ya existe en el sistema');
			}
		else
			{
				$sql="insert into renglon (renglon, id_subgrupo, ident_renglon) 
						values ('".$_POST['renglon']."',".$_POST['subgrupo'].",'".$_POST['ident_renglon']."') ";
	...
	if ($eli == 1)
	{
			...
			$selec = "delete from renglon where id_renglon = '$id'";
			echo $selec;
...
<form name="form1" method="post" action="renglon.php" onSubmit="return Verifica()">
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr bgcolor="#0066CC">
    <td colspan="7"><div align="center"><span class="Estilo1 Estilo2"> RENGLON </span></div></td>
    </tr>
  <tr>
  	<td>&nbsp;</td>
    ...
    <td class="Estilo7">
		...
		<? 
			$sql="select id_subgrupo, ident_subgrupo, subgrupo from subgrupo";
			$result  = mssql_query($sql);
			while ($row = mssql_fetch_array($result))
			 { ?>
			  	<option value="<? echo $row['id_subgrupo']; ?>"><? echo $row['ident_subgrupo'].' '.$row['subgrupo']; ?></option>
		<?	 }
		?>
		</select>
	</td>
  ...
  <tr class="Estilo1" >
    ...
    <td class="Estilo7"><input name="ident_renglon" type="text" class="Estilo7" maxsize="3"  size="3" onKeyUp="javascript:this.value=this.value.toUpperCase();"></td>
  </tr>


  <tr class="Estilo1" >
    <td class="Estilo22" align="right">Concepto <font color="#FF0000"><strong>**</strong></font></td>
    <td class="Estilo7"><input name="renglon" type="text" class="Estilo7" maxsize="150"  size="100" onKeyUp="javascript:this.value=this.value.toUpperCase();"></td>
  </tr>
...
<table border="0" align="center" class="Estilo1 ">
  <tr>
    ...
    <td bgcolor="#99CCFFF" align="center"><strong>SubGrupo</strong></td>
	<td bgcolor="#C9CDED" align="center"><strong>Rengl�n</strong></td>
    <td bgcolor="#99CCFFF">&nbsp;</td>
...
//	$sqlsel = "select id_grupo, grupo from grupo where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
		//	$sqlsel = "select c.id_renglon, a.grupo, a.ident_grupo, b.id_subgrupo, b.subgrupo, b.ident_subgrupo, c.ident_renglon, c.renglon from grupo a, subgrupo b, renglon c where b.id_subgrupo = c.id_subgrupo and a.id_grupo = b.id_grupo ";//where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
//	echo $sqlsel;
//	$sqlsel = "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id_grupo) as row FROM grupo ) as grupo WHERE row > $rangoini and row <= $rangofin";
	...
	<tr>
		...
		<td bgcolor="#C9CDED"><? echo $row["ident_renglon"].' '.$row["renglon"]; ?></td>
		
		<!--td bgcolor="#C9CDED"><a href="edicion_subgrupo.php?tabla=subgrupo&pag=subgrupo.php&id=<? //echo $row["id_renglon"]; ?>" title="Editar" target="mainFrame"><img src="images/iconos/b_edit.png" width="16" height="16" border="0"></a></td-->
		
		<td bgcolor="#99CCFF"><a href="editar_renglon.php?codigo=<? echo $cod; ?>"> <img src="images/iconos/b_edit.png" width="16" height="16" border="0"></a></td>
		
		
	    <!--td bgcolor="#99CCFF"><a href="renglon.php?eli=1&id=<?//echo $row["id_renglon"]; ?>" title="Eliminar" target="self"><img src="images/iconos/button_drop.png" width="16" height="16" border="0"></a></td-->
	</tr>
	<?
 		}
  ...
  <table border="1" align="center">
    <tr>
		<?
			if ($PagNow != 1)
			{
				echo "<td><a href=\"validapag.php?linkant=renglon.php&pag=-1&maxpag=".$maxpag."\">Anterior</a></td>";
			}
			if ($maxpag > 10)
			{
				for ($contpag = $PagNow;($contpag <= $maxpag) && ($contpag <= ($PagNow + 9));$contpag++)
				{ 
					echo "<td><a href=\"validapag.php?linkant=renglon.php&pag=".$contpag."&maxpag=".$maxpag."\">".$contpag."</a></td>";
				}
			}
			else
			{
			...
			if ($PagNow != $maxpag)
			{
				echo "<td><a href=\"validapag.php?linkant=renglon.php&pag=0&maxpag=".$maxpag."\">Siguiente</a></td>";
			}
		?>  
    </tr>
  </table>
