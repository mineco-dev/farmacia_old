...
// Valores iniciales para la paginacion
	...
	$rangofin = 10;
	$sqlsel = mssql_query("select id_fuente, fuente, denominacion from fuente_financiamiento");
...
//	 cambiar_ventana('mtlogin.php');
 ...
 if ( isset($_POST['inserta']) && ($_POST['inserta'] == 1) ) 
	{
		$verifica = "select denominacion from fuente_financiamiento where denominacion= '".$_POST['denominacion']."'";
		$resver = mssql_query($verifica);
		if (mssql_num_rows($resver) > 0) 
			{
			 envia_msg('La fuente que desea ingresar ya existe en el sistema');
			}
		else
			{
				$sql="insert into fuente_financiamiento (fuente, denominacion, clasifica) 
						values ('".$_POST['fuente']."','".$_POST['denominacion']."','".$_POST['clasifica']."') ";
...
<form name="form1" method="post" action="fuente.php" onSubmit="return Verifica()">
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr bgcolor="#0066CC">
    <td colspan="7"><div align="center"><span class="Estilo1 Estilo2"> FUENTE DE FINANCIAMIENTO</span></div></td>
    </tr>
  <tr>
  	<td>&nbsp;</td>
  ...
  <tr class="Estilo1" >
    <td class="Estilo22" align="right">Fuente<font color="#FF0000"><strong>**</strong></font></td>
    <td class="Estilo7"><input name="fuente" type="text" class="Estilo7" maxsize="1"  size="1" onKeyUp="javascript:this.value=this.value.toUpperCase();">
    
	Agregada<font color="#FF0000"><strong>**</strong></font>
	  <input name="clasifica" type="radio" value="1">
	Financiamiento<font color="#FF0000"><strong>**</strong></font>
		  <input name="clasifica" type="radio" value="2">
	  
	  </td>
  </tr>

  <tr class="Estilo1" >
    <td class="Estilo22" align="right">Denominación<font color="#FF0000"><strong>**</strong></font></td>
    <td class="Estilo7"><input name="denominacion" type="text" class="Estilo7" maxsize="50"  size="50" onKeyUp="javascript:this.value=this.value.toUpperCase();"></td>
  </tr>
...
<table border="0" align="center" class="Estilo1 ">
  <tr>
    <td bgcolor="#C9CDED" align="center"><strong>Fuente</strong></td>
    <td bgcolor="#99CCFFF" width="200" align="center"><strong>Denominacion</strong></td>
...
//	$sqlsel = "select id_grupo, grupo from grupo where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
	$sqlsel = "select id_fuente, fuente, denominacion from fuente_financiamiento order by fuente asc";//where id_grupo >= $rangoini and id_grupo <= $rangofin order by 1";
//	echo $sqlsel;
//	$sqlsel = "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id_grupo) as row FROM grupo ) as grupo WHERE row > $rangoini and row <= $rangofin";
	...
	?>
	<tr>
		<td bgcolor="#C9CDED"><? echo $row["fuente"]; ?></td>
		<td bgcolor="#99CCFF"><? echo $row["denominacion"]; ?></td>
		
		
		<!--td bgcolor="#C9CDED"><a href="edicion_grupo.php?tabla=grupo&pag=grupo.php&id=<? // echo $row["id_fuente"]; ?>" title="Editar" target="mainFrame"><img src="images/iconos/b_edit.png" width="16" height="16" border="0"></a></td-->
		
		
		<td bgcolor="#C9CDED"><a href="editar_fuente.php?codigo=<? echo $cod; ?>"> <img src="images/iconos/b_edit.png" width="16" height="16" border="0"></a></td>
		
		
	    <!--td bgcolor="#99CCFF"><a href="fuente.php?eli=1&id=<? //echo $row["id_fuente"]; ?>" title="Eliminar" target="mainFrame"><img src="images/iconos/button_drop.png" width="16" height="16" border="0"></a></td-->
	</tr>
	<?
 		}
  ...
  <table border="1" align="center">
    <tr>
		<?
			if ($PagNow != 1)
			{
				echo "<td><a href=\"validapag.php?linkant=fuente.php&pag=-1&maxpag=".$maxpag."\">Anterior</a></td>";
			}
			if ($maxpag > 10)
			{
				for ($contpag = $PagNow;($contpag <= $maxpag) && ($contpag <= ($PagNow + 9));$contpag++)
				{ 
					echo "<td><a href=\"validapag.php?linkant=fuente.php&pag=".$contpag."&maxpag=".$maxpag."\">".$contpag."</a></td>";
				}
			}
			else
			{
			...
			if ($PagNow != $maxpag)
			{
				echo "<td><a href=\"validapag.php?linkant=fuente.php&pag=0&maxpag=".$maxpag."\">Siguiente</a></td>";
			}
		?>  
    </tr>
  </table>
