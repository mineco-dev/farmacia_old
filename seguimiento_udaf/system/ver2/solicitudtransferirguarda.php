...
//ENVIO HACIA GERENCIA
	...
	if (strlen($descripcion)>0)
	{
		...
		$dbms->sql = "update docs_udaf set estado = 4 where estado = 2 and id_documento = $id_documento";
		$dbms->Query();
		
		//$idusuario=$_REQUEST["idusuario"];
		
		$dbms2=new DBMS(conectardb($bitacora2));	
				  	$dbms2->bdd=$database_cnn;
                    $query = "select * from usuario where activo=1 and codigo_usuario='$idusuario' /*(831,768,655)*/";
                    $dbms2->sql=$query;
                    $dbms2->Query();
										...
										{	
											...
											$mensaje = 'Esta es una notificación autom�tica de Documentos de la Unidad de Administracion Financiera, para ver la solicitud favor de ingresar a <a href="http://aseggys.mineco.gob.gt">aseggys.mineco.gob.gt</a>, usted tiene el expediente No.$Fields["id_documento"]';
