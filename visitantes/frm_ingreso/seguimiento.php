		...
		echo '<table>';
		$obj2=41;
		if (isset($obj2)) //verifico si hay objeto seleccionado
		{			
			...
			{										
				...
				$qry_plantilla="SELECT c.codigo_campo, c.campo, c.codigo_tipo_campo, c.tb_origen, c.validar, c.texto_validacion, c.ayuda, c.cambio_fila, 
								...
								where p.codigo_formulario='$obj2' and c.activo=1
								order by orden"; 																									
				...
				{		
					...
					$campo[$cnt2]=$row_qry_plantilla["campo"];				
					$tipo_campo2[$cnt2]=$row_qry_plantilla["codigo_tipo_campo"];
				...
				while($cnt2<=count($campo))  //para que devuelva el contenido de los campos del registro que se esta editando
				{	
					$campotemp=$campo[$cnt2];					
					if (($tipo_campo2[$cnt2]==8) || ($tipo_campo2[$cnt2]==10)) $campotemp="convert(nvarchar, $campo[$cnt2], 126) as $campo[$cnt2]";
					if ($cnt2==count($campo)) $qry_item_catalogo.=" $campotemp ";
					else $qry_item_catalogo.=" $campotemp, ";
					$cnt2++;
				}
				$qry_item_catalogo.="from $tabla_destino where $campo_llave_destino=$ids";				
				$res_qry_plantilla=$query($qry_plantilla);	
				...
				{
					while ($cnt2<=count($campo))  // arreglo que contiene el contenido del registro editado
					{
						$contenido2[$cnt2]=$row_qry_item_catalogo["$campo[$cnt2]"];						
						$cnt2++;
					}				
				}
				$item2=2;
				...
				{							
					...
					if (($row_qry_plantilla["cambio_fila"]==1) || ($row_qry_plantilla["cambio_fila"]==3)) 
					{
						...
						{
							echo '<input name="'.$row_qry_plantilla["campo"].'" type="text" id="'.$row_qry_plantilla["campo"].'" size="'.$row_qry_plantilla["tamano"].'" value="'.$contenido2[$item2].'" '.$status.'>';
						}
						else
						if ($row_qry_plantilla["codigo_tipo_campo"]==2)
						{
							...
							$qry_cbo="SELECT * FROM $latabla where $campollave='$contenido2[$item2]' order by $campoorigen";				 																																$res_qry_cbo=$query($qry_cbo);
							while($row_qry_cbo=$fetch_array($res_qry_cbo))
							{
								$valoractual=$row_qry_cbo["$campoorigen"];
								$codigo_valoractual=$row_qry_cbo["$campollave"];							
							}
							$qry_cbo="SELECT * FROM $latabla where $campollave<>'$contenido2[$item2]' and $condicion order by $campoorigen"; 	
						...
						{																	
							...
							if ($contenido2[$item2]!="NA")						
							{
								echo '<a href="archivos/'.$contenido2[$item2].'"><img src="../../../images/iconos/ico_ver.jpg" alt="Para ver el archivo que contiene el detalle de este equipo, pulse aquí" border="0"></a>';
								echo('<input name="'.$row_qry_plantilla["campo"].'_temp" type="hidden" id="'.$row_qry_plantilla["campo"].'_temp"  value="'.$contenido2[$item2].'"/>');
							}	
						}				
						else
					if ($row_qry_plantilla["codigo_tipo_campo"]==8)
					{	
						$day=substr($contenido2[$item2],8,2);
						$month = strtolower(substr($contenido2[$item2],5,2));
						$year = substr($contenido2[$item2],0,4);
						if ($status=='disabled')
						{
							echo $day.'-'.$month.'-'.$year;
						}	
					...
					{					
						echo '<textarea name="'.$row_qry_plantilla["campo"].'" cols="'.$row_qry_plantilla["tamano"].'" rows="4" '.$status.'>'.$contenido2[$item2].'</textarea>';
					}
					else
					...
					{							
						...
						$month = strtolower(substr($contenido2[$item2],5,2));
						$year = substr($contenido2[$item2],0,4);
						$hour = substr($contenido2[$item2],11,2);
						$min = substr($contenido2[$item2],14,2);
						if ($status=='disabled')
						{
					...
					{					
						conectardb($rrhh);
						$id_solicitante_actual=$contenido2[$item2];
