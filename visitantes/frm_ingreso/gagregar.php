...
<table width="100%"  border="0">
  ...
  <tr>
		...
		if (($acceso==2) || ($acceso==3) || ($acceso==6) || ($acceso==7) || ($acceso==8))
		{
			...
			conectardb($visitantes);
				...
				if (!$registro_encontrado)
				{
						...
						if ($res_insert)
						{
							$qry_ultimo='select max(codigo_visitante) as ultimo from seg_visitante';
							$res_ultimo=$query($qry_ultimo);
							while($row_qry_ultimo=$fetch_array($res_ultimo))
							{
								$ultimo_visitante=$row_qry_ultimo["ultimo"];
							}
							echo '<tr><td class="titulocategoria" colspan="3" align="center">EL REGISTRO SE HA GRABADO CORRECTAMENTE<br><br>Para ingresar otro nuevo visitante <a href="agregar.php">[HAGA CLIC AQUI]</a><br><br>SI DESEA REGISTRAR LA VISITA <a href="ingreso.php?id='.$ultimo_visitante.'">HAGA CLIC AQUI <img src="../../../images/iconos/ico_entrar.gif" alt="Registrar visita" border="0"> </a></td></tr>';															
	...
	{
		echo '<tr><td class="error" colspan="3" align="center">Para ingresar otro registro <a href="agregar.php">[HAGA CLIC AQUI]</a></td></tr>';
	}
	?>            
	</td>
  </tr>
