...
<table width="100%"  border="0">
  <tr>
    ...
    <td width="77%"><p align="center" class="titulocategoria Estilo1">SUBGERENCIA ADMINISTRATIVA</p>
    <p align="center" class="titulocategoria">M&Oacute;DULO: VISITANTES</p></td>
    <td width="12%"><div align="right"><img src="../../images/visitantes.gif" width="124" height="96"></div></td>
  </tr>
  <tr>
  ...
  <tr>
				...
				if (!$registro_encontrado)
				{
						...
						if ($res_insert)
						{
							...
							// insertar empleados visitados
				  		    $cnt=1; 		
					  		$ref=md5($ultima_visita);
							while($cnt<=count($bien))
							  {						
									$visitado=$bien[$cnt][1];
									$qry_visitado ="insert into seg_visita_det";	
									$qry_visitado.="(codigo_visita, codigo_usuario, codigo_estado, referencia) ";
									$qry_visitado.="values ($ultima_visita, '$visitado', 1, '$ref')";			
									$query($qry_visitado);									
									$cnt++;										
							  }	
								...
								while($cnt<=count($bien))
								 {		
										...
										{	
											...
											//$mensaje='prueba';
											$mensaje = 'En este momento el(la) se�or(a) '.$txt_visitante.' ingres� al Ministerio indicando que lo visitar�a a usted, para confirmar su visita <a href="http://aseggys.mineco.gob.gt/visitantes/frm_salida/uconfirma.php?id='.$visitado.'&rf='.$ref.'">HAGA CLIC AQUI</a> de lo contrario comuniquese a Seguridad a la extensión 1000.';
	$titulo = "NOTIFICACION AUTOMATICA DE VISITA";
	envio($nomb_destino,$mail_destino,$nomb_remitente,$mail_remitente,$mensaje,$titulo,"","");
										}									
										$cnt++;			
								 }  // FIN DE confirmacion por correo						  
							echo '<tr><td class="titulocategoria" colspan="3" align="center">EL REGISTRO SE HA GRABADO CORRECTAMENTE</td></tr>';															
						} // fin del ingreso del encabezado
						else
						{
							echo '<tr><td class="error" colspan="3" align="center">Se produjo un error al grabar, El registro NO se ha grabado<br><br>Para intentar nuevamente <a href="agregar.php">[HAGA CLIC AQUI]</a></td></tr>';
						}
	...
	{
		echo '<tr><td class="error" colspan="3" align="center">CONTINUE UTILIZANDO LAS OPCIONES QUE APARECEN A LA IZQUIERDA</td></tr>';
	}
	?>            
	</td>
  </tr>
