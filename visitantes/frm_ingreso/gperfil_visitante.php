...
<table width="100%"  border="0">
  ...
  <tr>
	...
	{			
		...
		if (($acceso>=4) && ($acceso<=8))
		{
			...
			while ($cnt<=count($campo)) //concatena lista de campos a grabar
				...
				if ($res_insert)
				{					
					$qry_inserta_comentario="update seg_visita_det set codigo_motivo='$codigo_motivo', especifique_motivo='$especifique_motivo', usuario_comento='$nombre_usuario', fecha_comento=getdate() where codigo_visita_det='$txt_ids'";
					$query($qry_inserta_comentario);					
					
					echo '<tr><td class="titulocategoria" colspan="3" align="center">EL REGISTRO SE HA ACTUALIZADO CORRECTAMENTE<br><br></td></tr>';															
				} // fin del ingreso del encabezado
				else
				{
					echo '<tr><td class="error" colspan="3" align="center">Se produjo un error durante la actualización, El registro NO ha cambiado</td></tr>';
				}	
			}
			else
			{
	...
	{
		echo '<tr><td class="error" colspan="3" align="center">Para continuar seleccione alguna opción del men� izquierdo</td></tr>';
	}
	?>            
	</td>
  </tr>
