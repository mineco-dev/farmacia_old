		...
		{			
			...
			$qry_plantilla="SELECT c.codigo_campo, c.campo, c.codigo_tipo_campo, c.tb_origen, c.validar, c.texto_validacion, c.ayuda, c.incluir_en_detalle,
				...
				{	
					...
					$qry_condicion="WHERE seg_visita.codigo_estado<5 ";	
				$cnt=1;
				...
				$qry_detalle='select seg_visita.codigo_visita, seg_visitante.nombre_visitante, seg_visita.gafete_asignado, seg_visita.codigo_estado, seg_visitante.codigo_visitante
FROM seg_visitante INNER JOIN seg_visita ON seg_visitante.codigo_visitante=seg_visita.codigo_visitante '.$qry_condicion .' 
order by seg_visitante.nombre_visitante, seg_visita.gafete_asignado';																																	
		  ...
		  while($row_qry_insertados=$fetch_array($res_qry_insertados))
		  {
			  ...
			  while ($cnt<=$no_campos_detalle) 
			  {
				...
				if ($status==1)		
				{						 
				echo '<td class='.$clase.'><center><a href="../salida.php?id='.$row_qry_insertados[4].'&idv='.$row_qry_insertados[0].'"><img src="../../../images/iconos/ico_pend_confirmar.gif" alt="Aceptar esta visita" border="0">&nbsp;No confirmado</a></center></td>';  
				}
				else
				if ($status==2)		
				{						 
				echo '<td class='.$clase.'><center><a href="../salida.php?id='.$row_qry_insertados[4].'&idv='.$row_qry_insertados[0].'"><img src="../../../images/iconos/ico_salir.jpg" alt="Visita confirmada" border="0">Confirmado</a></center></td>';  
				
			  }	
			  else
				if ($status==3)		
				{						 
				echo '<td class='.$clase.'><center><a href="../salida.php?id='.$row_qry_insertados[4].'&idv='.$row_qry_insertados[0].'"><img src="../../../images/iconos/ico_trasladado.gif" alt="Visita trasladada de otra dependencia" border="0">Traslado</a></center></td>';  
