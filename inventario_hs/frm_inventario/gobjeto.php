	...
	{			
		if (isset($txt_obj)) //verifico si hay objeto seleccionado
		{
			...
			if ($existe==false)
			{
				...
				while ($cnt<=count($campo))  //concatena el contenido a grabar en los campos
				{
					...
					{						
						...
						{ 
							...
							$qry_ultimo_reg="select max(codigo_inventario_enc) as ultimo_registro  from tb_inventario";
							...
							if (move_uploaded_file($HTTP_POST_FILES["$campo[$cnt]"]['tmp_name'], "detallepc/".$nombre_archivo_def))
								{ 
								  ...
								  $variable=strtoupper($nombre_archivo_def);
								}
								else
								{ 
				...
				if ($res_insert)
				{
					echo '<tr><td class="titulocategoria" colspan="3" align="center">EL REGISTRO SE HA GRABADO CORRECTAMENTE<br><br>Para ingresar otro registro <a href="objeto.php">[HAGA CLIC AQUI]</a></td></tr>';
