...
<?	if (isset($hoy))
	...
	else
	{
		...
		{	
			if ($txt_buscar!="")
			{
				$consulta = "EXEC proc_publicacion_view @vcodigo_dependencia='$id', @vtipo_publicacion=3, @vpalabras_clave='%$txt_buscar%'";
			}
			else
			{
				$consulta = "EXEC proc_publicacion_view @vcodigo_dependencia='$id', @vtipo_publicacion=3, @vpalabras_clave='NA'";
			}		
		}
		else
		{
