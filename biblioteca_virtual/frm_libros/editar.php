...
<script src="../../includes/facefiles/jquery-1.2.2.pack.js" type="text/javascript"></script>
<link href="../../includes/facefiles/facebox.css" media="screen" rel="stylesheet" type="text/css" />
<script src="../../includes/facefiles/facebox.js" type="text/javascript"></script>
...
<table width="100%"  border="0">
  <tr>
    ...
    <td width="72%"><p align="center" class="titulocategoria Estilo1">BIBLIOTECA VIRTUAL Y TECNOL&Oacute;GICA - BIVITEC</p>
    <p align="center" class="titulocategoria"> MODULO DE: DOCUMENTOS Y LIBROS</p></td>
    <td width="10%"><div align="right"><img src="../../images/libros.jpg" width="128" height="87"></div></td>
  </tr>
  <tr>
  ...
  <tr>
    <td height="25" colspan="2"><div align="right" class="tituloproducto">
      <div align="center">FORMULARIO PARA VISUALIZACI&Oacute;N Y/O MODIFICACION DE DATOS        
      </div>
    </div></td>
    <td height="25">&nbsp;</td>
  </tr>
		...
		if (isset($obj)) //verifico si hay objeto seleccionado
		{
			...
			{							
				...
				{	
					...
					if ($cnt==count($campo)) $qry_item_catalogo.=" $campotemp, fecha_ingreso_txt ";
				...
				{											
					while ($cnt<=count($campo))  // arreglo que contiene el contenido del registro editado
					{
						...
						//print $contenido[$cnt]."<br>";
						$cnt++;						
					}			
					$cnt--;
					$fecha_temp=$row_qry_item_catalogo["$cnt"];
				}
				$item=1;
				while($row_qry_plantilla=$fetch_array($res_qry_plantilla))
				{							
					...
					echo '<td class="boxTitleBgLightGrey">';													
						...
						{				
							if ($contenido[$item]=='SI')
								echo '<input name="'.$row_qry_plantilla["campo"].'" type="checkbox" id="'.$row_qry_plantilla["campo"].'" value="'.$contenido[$item].'" checked '.$status.'>';
								else
								   echo '<input name="'.$row_qry_plantilla["campo"].'" type="checkbox" id="'.$row_qry_plantilla["campo"].'" value="'.$contenido[$item].'" '.$status.'>';
						}		
						else				
						...
						{																	
							...
							{
								$filename_len=strlen($contenido[$item])-3;
								$extension=substr(($contenido[$item]),$filename_len,3);
								if (($extension=='GIF') || ($extension=='JPG'))
									echo '<a href="archivos/'.$contenido[$item].'" rel="facebox"><img src="../../../images/iconos/ico_ver.jpg" alt="Para ver el archivo que contiene el detalle de este equipo, pulse aqu�" border="0"></a>';
								else
									echo '<a href="archivos/'.$contenido[$item].'"><img src="../../../images/iconos/ico_ver.jpg" alt="Para ver el archivo pulse clic aqui" border="0"></a>';
					...
					{	
						...
						{								
							...
							echo ' Fecha Ingresada: '.$fecha_temp;																																	
						}
					}	
					else
