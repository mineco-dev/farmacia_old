			...
			{							
				if (($acceso==6) || ($acceso==7)) $status='disabled'; else $status='';	
				{
							...
							conectardb($biblioteca);
								...
								$qry_insert="update $tabla_destino set activo='$st', usuario_rechazo='$nombre_usuario', fecha_rechazado=getdate() where $campo_llave_destino=$id";												
								session_unregister("ingresando_obj");					
								$res_insert=$query($qry_insert);			
								if ($res_insert)
								{					
									if ($st==3) $mensaje_estado="RECHAZADO";
