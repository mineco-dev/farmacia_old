...
<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="94%"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
  	...
  	?>
          <div align="right"><span class="tituloproducto">REPORTE DE INGRESOS </span><br>
              <? 
				...
				$encontro_registros=false;
				 ...
				 {				
					...
					$qry_categoria="select distinct(codigo_categoria), categoria from view_kardex 
									...
									codigo_tipo_movimiento=1 and codigo_categoria=$categoria";										
				 }							
				 else
				 {
				    $qry_categoria="select distinct(codigo_categoria), categoria from view_kardex 
					...
					{						
						...
						{										
							...
							$qry_subcategoria="select distinct(codigo_subcategoria), subcategoria from view_kardex 
											   where codigo_bodega=$rpt_bodega and fecha>='$inicio' and fecha<='$fin' and 
											   codigo_tipo_movimiento=1 and codigo_subcategoria=$subcategoria and codigo_categoria=$codigo_categoria";
						}
						else
						{
						    $qry_subcategoria="select distinct(codigo_subcategoria), subcategoria from view_kardex 
										      where codigo_bodega=$rpt_bodega and fecha>='$inicio' and fecha<='$fin' and 
										      codigo_tipo_movimiento=1 and codigo_categoria=$codigo_categoria";
						}
							...
							while($row_subcat=$fetch_array($res_qry_subcategoria))				
							{
								...
								{									
									...
									$qry_nombre_producto="select distinct(codigo_producto), descripcion from view_kardex 
														  where codigo_bodega=$rpt_bodega and fecha>='$inicio' and fecha<='$fin' and 
														  codigo_tipo_movimiento=1 and codigo_producto=$producto";
								}
								else								
								if (isset($_SESSION["subcategoria"]))
								{									
									$qry_nombre_producto="select distinct(codigo_producto), descripcion from view_kardex 
													  where codigo_bodega=$rpt_bodega and fecha>='$inicio' and fecha<='$fin' and 
													 codigo_tipo_movimiento=1 and codigo_subcategoria=$codigo_subcategoria";
								}
								else
								{
									    $qry_nombre_producto="select distinct(codigo_producto), descripcion from view_kardex 
														  where codigo_bodega=$rpt_bodega and fecha>='$inicio' and fecha<='$fin' and 
														  codigo_tipo_movimiento=1 and codigo_subcategoria=$codigo_subcategoria and codigo_categoria=$codigo_categoria";
								}
									...
									while($row_nombre_producto=$fetch_array($res_qry_nombre_producto))				
									{
										...
										$qry_producto="select * from view_kardex 
													   ...
													   and codigo_tipo_movimiento=1 and codigo_producto=$codigo_producto";  									    
										...
										{
											echo '<tr class="detalletabla4"><td colspan="5" align="right"><span class="descripcionproducto">TOTAL INGRESOS--><span class="titulomenu">'.$row_subtotal_xprod["subtotal"].'</span></td></tr>';	
