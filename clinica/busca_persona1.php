      ...
      <tr align="center" bgcolor="#006699" class="thead">
		...
		<?
				...
				$consulta = "SELECT (a.apellido +' '+ a.apellido2 +' '+ a.apellidocasada +', '+ a.nombre +' '+ a.nombre2 +' '+ a.nombre3) as empleado, a.activo, a.idasesor,
							...
							where (a.apellido like '%$busqueda%' or a.apellido2 like '%$busqueda%' or a.nombre like '%$busqueda%' or a.nombre2 like '%$busqueda%' or d.nombre like '%$busqueda%') and (a.activo=1)";
				...
				if (isset($_REQUEST["in"]))	
				{
					...
					if ($inicial!="all")
						$consulta = "SELECT (a.apellido +' '+ a.apellido2 +' '+ a.apellidocasada +', '+ a.nombre +' '+ a.nombre2 +' '+ a.nombre3) as empleado, a.activo, a.idasesor,
									...
									FROM asesor a INNER JOIN
                					direccion d ON a.iddireccion = d.iddireccion
									where a.apellido like '$inicial%' and a.activo=1";
				...
				else
				{
					$consulta =     "SELECT top 10(a.apellido +' '+ a.apellido2 +' '+ a.apellidocasada +', '+ a.nombre +' '+ a.nombre2 +' '+ a.nombre3) as empleado, a.activo, a.idasesor,
									...
									FROM asesor a INNER JOIN
                					direccion d ON a.iddireccion = d.iddireccion
							     	where a.apellido like 'a%' and a.activo=1";
				}
				conectardb($rrhh);
				...
				while($row=$fetch_array($result))
				{
					...
					echo '<td>'.$row["empleado"].'</td><td>'.$row["dependencia"].'</td></tr>';										
					$i++;
				}				
				$free_result($result);				
			 ?>
