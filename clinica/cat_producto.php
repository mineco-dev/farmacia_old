...
{	
	...
	{	
		conectardb($almacen);
		$nuevo_producto=strtoupper($_REQUEST["txt_producto"]);	
		...
		$minima=$_REQUEST["txt_minima"];	
		$maxima=$_REQUEST["txt_maxima"];	
		...
		{						
			...
			$qry_producto="INSERT INTO cat_producto(codigo_categoria, codigo_subcategoria, codigo_producto, producto, marca, codigo_medida, punto_reorden, existencia_maxima, codigo_estado, uso, activo, usuario_creo, fecha_creado) 
							VALUES ($categoria, $subcategoria, '$ult_cod_prod_mineco', '$nuevo_producto', '$marca', $presentacion, $minima, $maxima, 1, '$uso', 1,'$nombre_usuario', getdate())";
			$query($qry_producto);		
            ...
            <tr>
              ...
              <tr>
                <td height="25" colspan="3"><span class="tituloproducto">Categor&iacute;a
                  <?
				  	conectardb($almacen);
					$qry_categoria="SELECT distinct(s.codigo_categoria), s.categoria FROM cat_categoria s
										inner join tb_categoria_x_bodega c
										on c.codigo_categoria=s.codigo_categoria
										WHERE activo=1 and c.codigo_bodega='$bodega' ORDER BY s.codigo_categoria";	
              ...
              <tr>
                <td height="25" colspan="3"><span class="tituloproducto">
				<div id="Div_subcategoria">
		  		Subcategoria<select name="subcategoria"  id="select3">
		  		</select>
	 			 </div>
                  Presentaci&oacute;n
                  <?
              ...
              <tr>
                <td height="25" colspan="3"><span class="tituloproducto">Marca
                  <input name="txt_marca" type="text" id="txt_marca" size="40">
                </span>                  <div align="left"></div></td>
              </tr>
              <tr>
                <td height="25" colspan="3"><span class="tituloproducto">Existencia M&iacute;nima
                  <input name="txt_minima" type="text" id="txt_minima" size="10">
Existencia M&aacute;xima
<input name="txt_maxima" type="text" id="txt_maxima" size="10">
</span></td>
              </tr>
              <tr align="center" valign="top">
                <td height="25" colspan="3"><div align="left"><span class="tituloproducto">Uso
                  <input name="txt_uso" type="text" id="txt_uso" size="82">
                </span></div></td>
              </tr>
              <tr align="center" valign="top">
    ...
    <table class="tborder" cellpadding="0" cellspacing="1" border="0" width="100%" id="table17">
      <tr align="center" bgcolor="#006699" class="thead">
		...
		<?
				...
				$consulta = "SELECT p.rowid, b.codigo_categoria, b.codigo_bodega, p.codigo_subcategoria, p.codigo_producto, p.producto, p.marca, p.activo, m.unidad_medida FROM cat_producto p
							inner join cat_medida m on p.codigo_medida=m.codigo_medida
							inner join tb_categoria_x_bodega b
							on p.codigo_categoria=b.codigo_categoria and p.codigo_subcategoria=b.codigo_subcategoria							
							where b.codigo_bodega='$bodega' and producto like '%$busqueda%'";				
				}
				else	
				if (isset($_REQUEST["in"]))	
				{
					...
					if ($inicial!="all")
						$consulta = "SELECT p.rowid, b.codigo_categoria, b.codigo_bodega, p.codigo_subcategoria, p.codigo_producto, p.producto, p.marca, p.activo, m.unidad_medida FROM cat_producto p
									...
									inner join tb_categoria_x_bodega b
									on p.codigo_categoria=b.codigo_categoria and p.codigo_subcategoria=b.codigo_subcategoria
									where b.codigo_bodega='$bodega' and producto like '$inicial%'";
						else
							$consulta = "SELECT p.rowid, b.codigo_categoria, b.codigo_bodega, p.codigo_subcategoria, p.codigo_producto, p.producto, p.marca, p.activo, m.unidad_medida FROM cat_producto p
										inner join cat_medida m on p.codigo_medida=m.codigo_medida
										inner join tb_categoria_x_bodega b
										on p.codigo_categoria=b.codigo_categoria and p.codigo_subcategoria=b.codigo_subcategoria
										where b.codigo_bodega='$bodega'
							 			order by producto";
				}
				else
				{
					$consulta = "SELECT p.rowid, b.codigo_categoria, b.codigo_bodega, p.codigo_subcategoria, p.codigo_producto, p.producto, p.marca, p.activo, m.unidad_medida FROM cat_producto p
								...
								inner join tb_categoria_x_bodega b
								on p.codigo_categoria=b.codigo_categoria and p.codigo_subcategoria=b.codigo_subcategoria
								where b.codigo_bodega='$bodega' and producto like 'A%'";
				}
				conectardb($almacen);
				...
				while($row=$fetch_array($result))
				{
					...
					{										
					echo '<tr class='.$clase.'>';								
					echo '<td>'.$row["producto"].'</td><td>'.$row["marca"].'</td><td>'.$row["unidad_medida"].'</td><td><center><a href="editar_producto.php?id='.$row["rowid"].'"><img src="../images/iconos/ico_editar.jpg" alt="Modificar información"></a></center></td><td><center><a href="cambia_stat.php?id='.$row["rowid"].'&stat=2&ref=4"><img src="../images/iconos/ico_activo.gif" alt="Activo"></a></center></td></tr>';					
					}
					else
						echo '<tr class='.$clase.'><td>'.$row["producto"].'</td><td>'.$row["marca"].'</td><td>'.$row["unidad_medida"].'</td><td><center><a href="editar_producto.php?id='.$row["rowid"].'"><img src="../images/iconos/ico_editar.jpg" alt="Modificar información"></a></center></td><td><center><a href="cambia_stat.php?id='.$row["rowid"].'&stat=1&ref=4"><img src="../images/iconos/ico_desactivado.gif" alt="Desactivado"></a></center></td></tr>';										
					$i++;
				}
				$free_result($result);
			 ?>
