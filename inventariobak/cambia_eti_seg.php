...
//   $usr = 1;
   $existe = 0;
   			$sql= "select id_inventario from m_inventario 
					where id_registro_informatica = '$_POST[reg_it_new]' and id_anio_reg_it = '$_POST[anio_reg_it_new]' 
					and id_institucion_reg_it ='$_POST[insti_reg_it_new]'";
//echo $sql.'<br>';					
			$result = mssql_query($sql);
			$rsRows = mssql_query("select @@rowcount as rows");
			$rows = mssql_fetch_assoc($rsRows); 
//			envia_msg($rows);
//			envia_msg('pos1 -'.$rows['rows']);
			if ( $rows['rows'] > 0 )
			 {
			  $existe++;
			 }
//envia_msg('a'.$existe);							
   			...
   			$sql= "select id_inventario from bitacora_etiqueta 
					where ( (id_reg_it = '$_POST[reg_it_new]' and id_anio_reg_it = '$_POST[anio_reg_it_new]' 
					and id_institucion_reg_it ='$_POST[insti_reg_it_new]' ) 
					or (id_reg_it_old = '$_POST[reg_it_new]' 
					and id_anio_reg_it_old = '$_POST[anio_reg_it_new]' 
					and id_insti_reg_it_old ='$_POST[insti_reg_it_new]' ) )";
//echo $sql.'<br>';
			$result = mssql_query($sql);
			$rsRow = mssql_query("select @@rowcount as row");
			$row = mssql_fetch_assoc($rsRow); 
//			envia_msg('pos1 -'.$row['row']);			
			if ( $row['row'] > 0 )
			 {
			  $existe++;
			 }
...
//envia_msg('b'.$existe);					 
			if ( $existe > 0 )
			 {
			  error_msg('No se puede asignar la etiqueta porque ya fue asignada anteriormente');
			 }
			else
			 {
			 		$consulta_r = "EXEC proc_add_bitacora_reg_it 
								@v_inventario ='$_POST[idi]', 
								@v_anio_registro_informatica = '$_POST[anio_reg_it_new]', 
								@v_insti_registro_informatica = '$_POST[insti_reg_it_new]', 
								@v_registro_informatica = '$_POST[reg_it_new]', 
								@v_anio_registro_it_old = '$_POST[anio_reg_it]', 
								@v_insti_registro_it_old = '$_POST[insti_reg_it]', 
								@v_registro_it_old = '$_POST[reg_it]', 
								@v_motivo_cambio = '$_POST[descripcion]', 
...
//		if (form1.nombre.value == "" || form1.apellido.value == "" || form1.idregistro.value == "" || form1.cedula.value == "" || form1.iddepartamento.value == "" || form1.usuario.value = "" || form1.password.value == "" || form1.iddepartamento2 == value ""  )
		if (form1.reg_it_new.value == "" || form1.descripcion.value == "" )
			{
...
<form name="form1" method="post" action="cambia_eti_seg.php" onSubmit="return Verifica()">
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr bgcolor="#0066CC">
    <td colspan="7"><div align="center"><span class="Estilo1 Estilo2"> REEMPLAZO DE ETIQUETA DE SEGURIDAD EN EQUIPO DE COMPUTO </span></div></td>
    </tr>
	      <?
...
//echo $query;
 ...
 while ($rows = mssql_fetch_array($resulta))
  ...
  <tr class="Estilo1">
    ...
    <tr class="Estilo1">
	  <td class="Estilo22" align="right">Nueva Etiqueta de Seguridad <font color="#FF0000"><strong>**</strong></font>
	  <td class="Estilo7"><input type="hidden" name="anio_reg_it_new" maxlength="4" size="4" value="2008"><strong>2008</strong> -
	  <select name="insti_reg_it" disabled>
	  <? $query = "select id_institucion_reg_it, institucion_reg_it from cat_institucion_reg_it order by 2";
	     $result = mssql_query($query);
		 while($row = mssql_fetch_array($result))
	  ...
	  <input type="hidden" name="insti_reg_it_new" value="<? echo $rows['id_institucion_reg_it']; ?>">
	  - <input type="text" name="reg_it_new" size="6" maxlength="6" onkeypress="return validar(event)">
</td>
  </tr>


	<tr class="Estilo1" >
		<td class="Estilo22" align="right">Motivo por el cual se cambia la etiqueta<font color="#FF0000">**</font></td>
