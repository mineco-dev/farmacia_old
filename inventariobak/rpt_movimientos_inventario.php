...
/*	$this->Cell(0,4,'GUATEMALA, '.$_POST['fecha_final'],0,1,'R');	
	...
	$this->SetFont('Arial','B',8);	
	$this->Cell(0,4,'BITACORA DE ACTIVOS ASIGNADOS A EMPLEADOS',0,1,'C');		
	$this->SetFont('Arial','',8);	
	$this->Ln(2);
...
//CONVERT(varchar,a.fecha_registro,105) fecha_registro
 $sele_caso = "select a.descripcion, b.id_registro_inventario,
				c.nombre, c.nombre2, c.nombre3, c.apellido, c.apellido2, c.apellidocasada, 
				a.fecha_registro
				from bitacora_inv_emp a, m_inventario b, RRHH.dbo.asesor c
				where a.id_inventario = b.id_inventario and a.id_empleado = c.idasesor
...
$pdf->SetFont('Arial','B',8); 
	 ...
	 $pdf->Cell(35,4,"FECHA MOVIMIENTO",1,0,'C');	 
 	 $pdf->Cell(85,4," EMPLEADO ",1,0,'C');
	 $pdf->Cell(130,4,"DESCRIPCION ",1,1,'C');	 
$pdf->SetFont('Arial','',8); 	 
 while ($rowc = mssql_fetch_array($resultc))	
  {
		   $ind++;
	   ...
	   $pdf->MultiCell(130,4,$rowc['descripcion'],1);	 	   	   	   	   	   
