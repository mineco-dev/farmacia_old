...
function tc_setDay(objname, dvalue){
	...
	if(isDate(dvalue, date_array[1], date_array[0])){
		...
		obj.src = "calendar/calendar_form.php?objname="+objname.toString()+"&selected_day="+dvalue+"&selected_month="+date_array[1]+"&selected_year="+date_array[0];
...
function tc_setMonth(objname, mvalue){
	...
	if(isDate(date_array[2], mvalue, date_array[0])){
		...
		var obj = document.getElementById(objname+'_frame');
		obj.src = "calendar/calendar_form.php?objname="+objname.toString()+"&selected_day="+date_array[2]+"&selected_month="+mvalue+"&selected_year="+date_array[0];
...
function tc_setYear(objname, yvalue){
	...
	if(isDate(date_array[2], date_array[1], yvalue)){
		...
		var obj = document.getElementById(objname+'_frame');
		obj.src = "calendar/calendar_form.php?objname="+objname.toString()+"&selected_day="+date_array[2]+"&selected_month="+date_array[1]+"&selected_year="+yvalue;
...
function isDate(strDay, strMonth, strYear){
	...
	if (strYear.length != 4 || year==0 || year<minYear || year>maxYear){
		alert("Ingrese año valido de 4 digitos entre "+minYear+" y "+maxYear)
		return false
	}
	return true
}