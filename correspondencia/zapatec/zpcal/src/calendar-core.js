	...
	if (el.navtype == 50 && target != el) {
		...
		if (cal.getDateStatus) { 
			...
			if (el.className.indexOf("minute", 0) != -1) {
				...
				minute = range[i]; // new value ew minutes
 ...
 *   are clicked, the time part involved is determined and it i
 incremented or
	...
	if (this.numberMonths == 1) {
      ...
      if (this.params && this.params.titleHtml)
		...
		for (var l = 1; l <= rowsOfMonths; ++l) {
			for (var k = 1; (k <= this.monthsInRow) && ((l - 1) * this.monthsInRow + k <= this.numberMonths); ++k) {
				if (this.vertical) {
					validMonth = month - diffMonth + ((k - 1) * (rowsOfMonths - 1) + (l - 1) + ((lasstrow < k) ? (last_row) : (k - 1)));
				} else {
 ...
 * or to close the calendar when dee clicks outside it.
 */
