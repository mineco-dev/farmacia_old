...
//todo    return unless (ref($this) eq ref($other));

    $xf = $this->_xf_index;    # Store XF index assigned by Workbook.pm
...
function get_xf() {
    ...
    $atr_num     = ($this->_num_format != 0);
    $atr_fnt     = ($this->_font_index != 0);
    $atr_alc     =  $this->_text_wrap;
    $atr_bdr     = ($this->_bottom   ||
                    $this->_top      ||
                    $this->_left     ||
                    $this->_right);
//!!!
    $atr_pat     = ($this->_fg_color ||
                    $this->_bg_color ||
                    $this->_pattern);
    $atr_prot    = 0;
...
#
function set_bold($weight=false) {
...
function set_properties() {
    ...
    foreach ($properties as $key=>$value) {
        ...
        #
        if (preg_match('/\W/', $key)) {
//todo        die "Unknown method: \$this->set_$key\n" 
        }
        ...
        #
//todo: "!==false" is no good solution...
        if ($value!==false) {
...
//todo        die $@ if $@; # Rethrow the eval error.
    }
}
