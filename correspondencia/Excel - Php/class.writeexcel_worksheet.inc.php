...
/* orig:
    $this->_name              = $_[0];
    $this->_index             = $_[1];
    $this->_activesheet       = $_[2];
    $this->_firstsheet        = $_[3];
    $this->_url_format        = $_[4];
    $this->_parser            = $_[5];
    $this->_tempdir           = $_[6];
*/
...
function _initialize() {
    ...
    $fh=fopen(tempnam($this->_tempdir, "php_writeexcel"), "w+");
...
//todo        carp 'Header string must be less than 255 characters';
        return;
    }
...
function set_footer($string, $margin) {
    ...
    if (strlen($string) >= 255) {
//todo        carp 'Footer string must be less than 255 characters';
        return;
    }
...
#
function set_h_pagebreaks() {

    $_=func_get_args();

    $this->_hbreaks=array_merge($this->_hbreaks, $_);
}
...
#
function set_v_pagebreaks() {

    $_=func_get_args();

    $this->_vbreaks=array_merge($this->_vbreaks, $_);
}
...
//todo        carp "Zoom factor $scale outside range: 10 <= zoom <= 400";
        $scale = 100;
    }
...
function set_print_scale($scale=100) {
    ...
    # Confine the scale to Excel's range
    if ($scale < 10 || $scale > 400) {
//todo        carp "Print scale $scale outside range: 10 <= zoom <= 400";
        $scale = 100;
    }
...
function _sort_pagebreaks() {
    ...
    # Numerical sort
    $array=sort(array_keys($hash), SORT_NUMERIC);
...
//!!!
    ...
    if ($this->_byte_order) {
//TODO
        $xl_double = array_reverse($xl_double);
    }
...
function _store_externcount() {
    ...
    $cxals    = $_[0];           # Number of external references
...
function _store_setup() {
    ...
    if ($this->_byte_order) {
        $numHdr = array_reverse($numHdr);
        $numFtr = array_reverse($numFtr);
    }
...
function _position_image() {
    ...
    list($col_start, $row_start, $x1, $y1, $width, $height) = $_;
...
function _store_obj_picture() {
    ...
    $colL        = $_[0];    # Col containing upper left corner of object
    $dxL         = $_[1];    # Distance from left side of cell

    $rwT         = $_[2];    # Row containing top left corner of object
    $dyT         = $_[3];    # Distance from top of cell

    $colR        = $_[4];    # Col containing lower right corner of object
    $dxR         = $_[5];    # Distance from right of cell

    $rwB         = $_[6];    # Row containing bottom right corner of object
    $dyB         = $_[7];    # Distance from bottom of cell
