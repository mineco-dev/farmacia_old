...
<table width="100%"  border="0">
  <tr>
    <td colspan="8" bgcolor="#000000"><div align="center"><span class="Estilo1">Bandeja de Correspondencia Finalizada </span></div></td>
  </tr>
  <?
  session_start();
...
//		concat(right(q.fecha,2),'/',month(q.fecha),'/',year(q.fecha))
			$SQL = "SELECT d.docu,s.status,d.titulo FROM documento d, seguimiento s WHERE d.docu=s.docu and s.aquien = $usuario and s.carpet=2 group by d.docu order by s.fecha desc";
