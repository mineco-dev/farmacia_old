// $Id: pane.js 4828 2006-10-19 14:02:29Z shacka $
/*
 * <pre>
...
Zapatec.Pane.prototype.prepareHtml = function() {
	if(this.config.containerType.toLowerCase() == 'iframe'){
		...
		iframe.src = Zapatec.Pane.path + "pane_files/blank.html#" + this.id;
...
Zapatec.Pane.prototype.initPane = function(){
	if(this.config.containerType.toLowerCase() == 'iframe'){
		...
		if (protocolSeparatorPos != -1) {
			//retreiving the domainpa		var domainSeparatorPos = url.indexOf("/", protocolSeparatorPos + 3);
...
Zapatec.Pane.prototype.setPaneContent = function(content, type) {
	...
	if (this.config.containerType.toLowerCase() == "iframe" && this.getContainer().src.indexOf((Zapatec.Pane.path + "pane_files/blank.html#" + this.id).replace(/\.\.\//g, "")) < 0) {
		this.ready = false;
		this.getContainer().src = Zapatec.Pane.path + "pane_files/blank.html#" + this.id;
	}
