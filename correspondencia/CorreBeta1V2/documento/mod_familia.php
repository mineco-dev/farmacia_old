...
//		envia_msg($_POST['id1']);	 
		...
		$sql = "update familiaridad set
					primer_nombre = '".$_POST['primer_nombre'.$hijos]."', segundo_nombre = '".$_POST['segundo_nombre'.$hijos]."', tercer_nombre = '".$_POST['tercer_nombre'.$hijos]
					."', primer_apellido = '".$_POST['primer_apellido'.$hijos]."', segundo_apellido = '".$_POST['segundo_apellido'.$hijos]."', apellido_casada = '".$_POST['apellido_casada'.$hijos]."',
					 id_orden_cedula = $orden_ced, registro_cedula = $reg_ced, sexo = '".$_POST['sexo'.$hijos]."', fecha_nacimiento = '".$fecha_nac."'	where id_familiaridad = ".$_POST['id'.$hijos];		
				$result = mssql_query($sql);
//		print $sql;
...
/*	if ( $validos < $_POST['numhi'] )
  ...
  if ( $validos > 0 )
 	{
	  envia_msg($validos.' FAMILIAR ACTUALIZADO EXITOSAMENTE');
	  cambiar_ventana('actualiza_familia.php?numhi='.$_GET['numhi'].'&paramas='.$_GET['paramas']);
	}
 if ( $invalidos > 0 )
 	{
	  envia_msg($invalidos.' FAMILIAR NO PUDO SER ACTUALIZADO');
	}
 }	// fin if ($_GET['val'] == 1)
...
/*procedimiento de actulizar en el cual verifica si hay informacion de familiares para el empleado y los muestra*/ 
	$query = 'select id_familiaridad, primer_nombre, segundo_nombre, tercer_nombre, primer_apellido, segundo_apellido, apellido_casada, id_orden_cedula, registro_cedula, sexo, 
			 convert(varchar,fecha_nacimiento,105) fecha_nacimiento, id_asesor, fecha_registro, id_parentesco from familiaridad where id_familiaridad = '.$_GET['id'];
// echo $query;
...
<form name="form1" method="POST" action="mod_familia.php?val=1&numhi=<? echo $_GET['numhi']; ?>&paramas=<? echo $_GET['paramas']; ?>&id=<? echo $_GET['id']; ?>" onSubmit="return Verifica()">
<!--form name="form1" method="post" action="asesoringreso.php"-->
  ...
  <tr bgcolor="#0066CC">
    <td colspan="13"><div align="center"><span class="Estilo1 Estilo2">Edici&oacute;n Datos Personales de Familiares</span></div></td>
    </tr>
...
<tr class="Estilo1">
	<!--td-->
	<input type="hidden" name="id<? echo $hijos; ?>" size="10" maxlength="10" value="<? echo $row['id_familiaridad']; ?>">
	<!--/td-->
	<td>
      <input type="text" name="primer_nombre<? echo $hijos; ?>" size="15" maxlength="20" value="<?  echo $row['primer_nombre']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">	</td>
	<td>
	<input type="text" name="segundo_nombre<? echo $hijos; ?>" size="15" maxlength="20"  value="<?  echo $row['segundo_nombre']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">
	</td>
	<td>
	<input type="text" name="tercer_nombre<? echo $hijos; ?>" size="15" maxlength="20" value="<?  echo $row['tercer_nombre']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">
	</td>
	<td>
	<input type="text" name="primer_apellido<? echo $hijos; ?>" size="15" maxlength="20" value="<?  echo $row['primer_apellido']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">
	</td>
	<td>
	<input type="text" name="segundo_apellido<? echo $hijos; ?>" size="15" maxlength="20" value="<?  echo $row['segundo_apellido']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">
	</td>
	<td>
	<input type="text" name="apellido_casada<? echo $hijos; ?>" size="15" maxlength="20" value="<?  echo $row['apellido_casada']; ?>" onKeyUp="javascript:this.value=this.value.toUpperCase();">
	</td>
...
<option></option>
	...
	$i=1;
	 while ($i<=31)
	  {
	    if ( substr($row['fecha_nacimiento'],0,2) == $i)
			{
	  ?>			
...
<option></option>
	...
	$i=1;
	 while ($i<=12)
	  {
	  if (substr($row['fecha_nacimiento'],3,2) == $i)
			{
	  ?>			
			<option value="<? if ($i < 10) { echo '0'.$i; } else { echo $i; } ?>" selected><? echo $i; ?></option>
	  <?	}
...
<option></option>
	...
	$i=1920;
	 while ($i<=date('Y'))
	  {
		 if (substr($row['fecha_nacimiento'],6,4) == $i)
			{
	  ?>			
...
<td><input name="registro_cedula<? echo $hijos; ?>" type="text" class="Estilo7" id="cedula" size="8" maxlength="10" value="<?  echo $row['registro_cedula']; ?>"></td>
 <?  } // fin for 
  } // fin if
?>
</tr>
