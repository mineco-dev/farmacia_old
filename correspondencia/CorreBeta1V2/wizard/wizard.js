/*
 * $Id: wizard.js 4521 2006-09-21 13:07:11Z shacka $
...
function propChange(el) {
	switch (el.id) {
		...
		case "f_prop_timeInterval":
			flat_calendar.timeInterval = parseInt(el.value, 10);
		break;

		//multiple months related variables
		case "f_prop_numberMonths":
			flat_calendar.numberMonths= parseInt(el.value, 10);
			...
			if (flat_calendar.controlMonth > flat_calendar.numberMonths) {
				  //If it is, set it to last month
					flat_calendar.controlMonth = flat_calendar.numberMonths;
					document.getElementById("f_prop_controlMonth").value = flat_calendar.controlMonth;
			}
		...
		if (flat_calendar.monthsInRow > flat_calendar.numberMonths) {
				flat_calendar.monthsInRow = flat_calendar.numberMonths;
				document.getElementById("f_prop_monthsInRow").value = flat_calendar.monthsInRow;
		}
		break;

		case "f_prop_controlMonth":
			flat_calendar.controlMonth= parseInt(el.value, 10);
			//check that controlmonth is not bigger than numberMonths
			if (flat_calendar.controlMonth > flat_calendar.numberMonths) {
				  //If it is, set it to last month
		...
		case "f_pro		monthsInRow":
			var monthsInRow = parseInt(el.value, 10);
		if (isNaN(monthsInRow)) {//It's blank
			monthsInRow = flat_calendar.numberMonths;
		}
		flat_calendar.monthsInRow= monthsInRow;
		if (flat_calendar.monthsInRow > flat_calendar.numberMonths) {
		...
		case "f_prop_vertical":
			flat_calendar.vertical = el.checked;
		break;
	}
	flat_calendar.refresh();
};
