...
<table width="90%"  border="0" align="center">
  <tr>
    <td width="17%" height="25"><div align="left"></div></td>
    <td colspan="3"><p align="center" class="titulocategoria Estilo1">TOMA DE INVENTARIO</p>
    </td>
    <td width="13%"><div align="right"><img src="../images/hard_soft.jpg" width="55" height="56"></div></td>
  </tr>
  <tr>
  ...
  <tr>
    <td height="25" colspan="2"><div align="center">CATEGORIA</div></td>
    <td><div align="center">SUBCATEGORIA</div></td>
    <td height="25" colspan="2"> <div align="center">OBJETO</div></td>
  </tr>
  <tr>
...
{	
	$consulta=mssql_query("SELECT * FROM cat_categoria WHERE activo=1 ORDER BY nombre_categoria");	
...
<?	
	...
	echo '{';							
			...
			$i++;
			echo 'if ((form['txt_obj'].value) == 2)';
				echo '{';
					echo 'ban = 0; for (i=1;i<8;i++) { if (valor(form['marca['+i+']'])) ban = 1; } if (ban == 0) {alert('Debe especificar la memoria que tiene el CPU'); return};';
					echo 'ban = 0; for (i=1;i<8;i++) { if (valor(form['marcaproc['+i+']'])) ban = 1; } if (ban == 0) {alert('Indique las caracteristicas del procesador'); return};';
					echo 'ban = 0; for (i=1;i<16;i++) { if (valor(form['marcadisco['+i+']'])) ban = 1; } if (ban == 0) {alert('Indique las caracteristicas del disco duro'); return};';
					echo 'ban = 0; for (i=1;i<8;i++) { if (valor(form['tipolector['+i+']'])) ban = 1; } if (ban == 0) {alert('Escriba las unidades de lectura (Cdrom/dvdrom/floppy) de este equipo'); return};';
					echo 'ban = 0; for (i=1;i<100;i++) { if (valor(form['tipoinstall['+i+']'])) ban = 1; } if (ban == 0) {alert('Se requiere el detalle del software instalado en este equipo'); return};';
				echo '};';
			}
		echo 'if (confirm('�Esta acción guarda y finaliza el ingreso de datos para este objeto, desea continuar?')) form.submit();';
	echo '}';		//fin funcion validar
	echo '</script>';	
?>
