  ...
  <tr>
	...
	{		
		...
		if (($acceso==2) || ($acceso==3) || ($acceso==6) || ($acceso==7) || ($acceso==8))
		{
			...
			$cnt=1; 		
			while($cnt<=count($proveedor))
			{						
				
				$qry_presupuesto_ejecucion_cuatrimestral="insert into tb_ejecucion_presupuesto(codigo_asignacion_cuatrimestral,proveedor,no_orden,fecha,comprometido,devengado,activo) values ($txt_codigo_asignacion_cuatrimestral,  '$proveedor[$cnt]', $no_orden[$cnt], '$fecha[$cnt]', $txt_saldo_comprometido, $devengado[$cnt],1)";
