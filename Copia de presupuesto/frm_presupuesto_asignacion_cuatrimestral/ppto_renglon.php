...
if (validarEntero(document.all.monto_asignado.value)===true)
		{
...
<table width="90%" border="0" align="center">		  
	...
	if ($c == 1)
	{
	$qry_mes_cat = "select nombre_mes from cat_mes where codigo_mes<=4";
	$salida_mes_cat = mssql_query($qry_mes_cat);
	
	while($out_mes_cat = mssql_fetch_array($salida_mes_cat))
	{
		print ($out_mes_cat[0])."-------->";
	}
	 }
	if ($c == 2)
	{
	$qry_mes_cat = "select nombre_mes from cat_mes where codigo_mes>=5 and codigo_mes <=8";
	$salida_mes_cat = mssql_query($qry_mes_cat);
	...
	if ($c == 3)
	{
	$qry_mes_cat = "select nombre_mes from cat_mes where codigo_mes>=9 and codigo_mes <=12";
	$salida_mes_cat = mssql_query($qry_mes_cat);
   ...
   <script>
function sumar() {
var n1 = parseInt(document.form1.monto_asignado1.value);
var n2 = parseInt(document.form1.monto_asignado2.value);
var n3 = parseInt(document.form1.monto_asignado3.value);
var n4 = parseInt(document.form1.monto_asignado4.value);
document.form1.monto_asignado.value=n1+n2+n3+n4;
}
</script>
    
				<div class="TabbedPanelsContentGroup">					
					<div class="TabbedPanelsContent">
					  <input type="text_mes1" name="monto_asignado1" id="monto_asignado1" size="10"<? echo $asignado1; ?>>
					  <input type="text_mes2" name="monto_asignado2" id="monto_asignado2" size="10"<? echo $asignado2; ?>>
					  <input type="text_mes3" name="monto_asignado3" id="monto_asignado3" size="10"<? echo $asignado3; ?>>
					  <input type="text_mes4" name="monto_asignado4" id="monto_asignado4" size="10"<? echo $asignado4; ?>>
					  <?  //include("presupuesto_renglon_det.php"); ?>
						<br>
					</div>											
				</div>
			</div>
          <td><input type="text" name="monto_asignado"  id="monto_asignado" size="10" disabled <? echo $asignado; ?> ></td>
	...
	<tr>
     ...
     <td><center><input name="cmd_guardar" type="button" onClick="validar(this.form)" id="cmd_guardar" value="Guardar" <? echo $status?> > </center></td>
     <td><left>
     <input type="button" value="Monto Asignado" onclick="sumar()">
