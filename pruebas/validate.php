...
{ 
   $vari4able = addslashes(trim($variable)); 
   return $variable; 
} 
...
$consulta = "Select * from helpdesk.dbo.usuario
inner join rrhh.dbo.tb_jefes_depen
on helpdesk.dbo.usuario.codigo_usuario = rrhh.dbo.tb_jefes_depen.codigo_usuario
where helpdesk.dbo.usuario.nombre_usuario = '$username'
and helpdesk.dbo.usuario.contrasena = '$password' and helpdesk.dbo.usuario.activo = 1";
 //print($consulta2);
...
$result=mssql_query($consulta);
	while($row=mssql_fetch_array($result))  
	{
		  	$user_cod=$row['codigo_usuario'];
			//print($user_cod);
			$nombre_jefe=$row['Nombre_Jefe_Depen'];						
            // print($nombre_jefe);
...
/*
session_register('nombre_jefe');
$_SESSION['nombre_jefe'] = $nombre_jefe;
session_register('codigo_dependencia');
$_SESSION['codigo_dependencia'] = $codigo_dependencia;
session_register('codigo_grupo');
